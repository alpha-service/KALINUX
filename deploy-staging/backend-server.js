...
let customers = [
  { id: 1, name: "Jean Dupont", email: "jean.dupont@email.com", phone: "+32 475 12 34 56", address: "Rue de la Paix 123, 1000 Brussels", vat_number: "BE0123456789", peppol_id: "0196:BE0123456789", credit_balance: 0, language: "fr" },
  { id: 2, name: "Marie Martin", email: "marie.martin@email.com", phone: "+32 475 98 76 54", address: "Avenue Louise 45, 1050 Brussels", vat_number: "BE0987654321", peppol_id: null, credit_balance: 0, language: "nl" },
  { id: 3, name: "Pierre Dubois", email: "pierre.dubois@email.com", phone: "+32 475 11 22 33", address: "Chaussée de Waterloo 78, 1060 Brussels", vat_number: "BE0111222333", peppol_id: "0196:BE0111222333", credit_balance: 0, language: "fr" },
];
...
app.put('/api/categories/:id', (req, res) => {
  console.log('PUT /api/categories/:id', req.params.id);
  try {
    ...
    categories[index] = {
      ...categories[index],
      ...req.body,
      id: categories[index].id, // Ensure ID doesn't change
      product_count: categories[index].product_count // Persist count
    };
...
app.get('/api/reports/dashboard', (req, res) => {
  ...
  try {
    ...
    console.log(`Found ${filteredSales.length} sales, ${filteredDocs.length} documents, and ${filteredReturns.length} returns`);

    // Calculate totals (subtracting returns)
    const salesTotal = filteredSales.reduce((sum, s) => sum + (s.total || 0), 0) +
      filteredDocs.reduce((sum, d) => sum + (d.total || 0), 0);

    const returnsTotal = filteredReturns.reduce((sum, r) => {
      return sum + r.lines.reduce((lSum, l) => lSum + (l.qty_returned * (l.unit_price_excl_vat || 0) * (1 + (l.vat_rate || 21) / 100)), 0);
    }, 0);

    const totalSales = salesTotal - returnsTotal;
    const transactionsCount = filteredSales.length + filteredDocs.length + filteredReturns.length;
    ...
    filteredReturns.forEach(r => {
      if (r.lines) {
        productsSold -= r.lines.reduce((sum, line) => sum + (line.qty_returned || 0), 0);
      }
    });
    ...
    while (currentDate <= endDate) {
      ...
      const dayReturns = filteredReturns
        .filter(r => {
...
app.get('/api/documents', (req, res) => {
  ...
  // Helper to calculate return total (TTC)
  const calculateReturnTotal = (r) => {
    return r.lines.reduce((sum, l) => {
      const lineTotal = (l.qty_returned || 0) * (l.unit_price_excl_vat || 0);
      const vat = lineTotal * ((l.vat_rate || 21) / 100);
      return sum + lineTotal + vat;
    }, 0);
  };

  // Merge regular documents, returns, and credit notes
  let result = [
    ...documents.map(d => ({ ...d })), // Keep regular docs as is (integer IDs)
    ...returns.map(r => ({
      ...r,
      id: `ret-${r.id}`, // Prefix ID to avoid collision
      original_id: r.id,
      doc_type: 'return',
      date: r.created_at,
      total: calculateReturnTotal(r),
      paid_total: 0,
      items: r.lines.map(line => ({
        ...line,
        name: line.description, // Map description to name for frontend compatibility
        qty: line.qty_returned, // Map qty_returned to qty for frontend compatibility
        unit_price: line.unit_price_excl_vat // Map unit_price_excl_vat to unit_price
      }))
    })),
    ...creditNotes.map(cn => ({
      ...cn,
      id: `cn-${cn.id}`, // Prefix ID to avoid collision
      original_id: cn.id,
      doc_type: 'credit_note', // Ensure doc_type is set
      date: cn.issue_date,
      total: cn.total_incl_vat, // Map total_incl_vat to total
      paid_total: cn.settlement_status === 'settled' ? cn.total_incl_vat : 0,
      items: cn.lines // Map lines to items for frontend compatibility
    }))
  ];
...
app.get('/api/documents/:id', (req, res) => {
  ...
  if (id.startsWith('ret-')) {
    const originalId = id.replace('ret-', '');
    const ret = returns.find(r => r.id == originalId);
    if (ret) {
      doc = {
        ...ret,
        id: id,
        ...
        doc_type: 'return',
        date: ret.created_at,
        total: ret.lines.reduce((sum, l) => {
          const lineTotal = (l.qty_returned || 0) * (l.unit_price_excl_vat || 0);
          const vat = lineTotal * ((l.vat_rate || 21) / 100);
          return sum + lineTotal + vat;
        }, 0),
        items: ret.lines // Map lines to items
      };
    }
  } else if (id.startsWith('cn-')) {
    const originalId = id.replace('cn-', '');
    const cn = creditNotes.find(c => c.id == originalId);
    if (cn) {
      doc = {
        ...cn,
        id: id,
        ...
        date: cn.issue_date,
        total: cn.total_incl_vat,
        paid_total: cn.settlement_status === 'settled' ? cn.total_incl_vat : 0,
        items: cn.lines // Map lines to items
      };
    }
  } else {
    // Standard document
...
app.post('/api/documents/:id/send-email', async (req, res) => {
  console.log('POST /api/documents/:id/send-email', req.params.id);
  const { id } = req.params;
  const { email, pdfBase64, subject, message, smtpConfig } = req.body;
  ...
  try {
    ...
    // Use provided SMTP config or default to a dummy one
    ...
    const info = await transporter.sendMail({
      from: smtpConfig?.from || '"ALPHA POS" <pos@kruhn.eu>',
      to: email,
      subject: subject || `Document ${doc.number}`,
      text: message || `Veuillez trouver ci-joint votre document ${doc.number}.`,
      attachments: [
        {
...
// Delete return (only if draft)
app.delete('/api/returns/:id', (req, res) => {
  console.log('DELETE /api/returns/:id', req.params.id);
  ...
  if (ret.status !== 'draft') {
    return res.status(400).json({ error: 'Cannot delete validated return' });
  }
...
// Send Invoice to Peppol via Peppyrus
app.post('/api/invoices/:id/send-peppol', async (req, res) => {
  console.log('POST /api/invoices/:id/send-peppol', req.params.id);

  if (!peppyrusSettings.api_key) {
    return res.status(400).json({ error: 'Peppyrus API Key not configured' });
  }
  ...
  // Validate customer has Peppol ID (or VAT)
  if (!customer.peppol_id && !customer.vat_number) {
    return res.status(400).json({ error: 'Customer needs a Peppol ID or VAT number' });
  }

  // Generate UBL
  try {
    const ublXml = generatePeppolInvoiceUBL(invoice, customer, companySettings);

    // Send to Peppyrus
    // Peppyrus expects JSON with ubl content? Or specific fields?
    // Based on generic "send/receive" via API, usually it's a wrapper.
    // Let's assume a wrapper object: { format: 'ubl', xml: ... } or similar.
    // If we strictly follow the service implementation we did:
    const payload = {
      documentType: 'Invoice',
      format: 'UBL',
      content: ublXml, // Base64 encoding might be needed, but let's try raw string first or Base64 if standard
      receiverId: customer.peppol_id || `0196:${customer.vat_number.replace(/[^0-9]/g, '')}`
    };

    const result = await peppyrusService.sendInvoice(payload, peppyrusSettings.api_key, peppyrusSettings.test_mode);

    // Update invoice status/log (mock)
    invoice.peppol_status = 'sent';
    invoice.peppol_sent_at = new Date().toISOString();

    res.json({ success: true, result });
  } catch (error) {
    console.error("Peppol Send Error:", error);
    res.status(500).json({ error: error.message || 'Failed to send to Peppol' });
  }
});
