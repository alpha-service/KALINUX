...
const PDF_TRANSLATIONS = {
  fr: {
    ...
    draft: "BROUILLON",
    payment: "Paiement",
    terms_invoice: "Paiement à 30 jours à compter de la date de facture, sauf accord écrit contraire.",
    terms_quote: "Devis valable 14 jours, sauf indication contraire.",
    page: "Page"
  },
  nl: {
    invoice: "FACTUUR",
    ...
    draft: "CONCEPT",
    payment: "Betaling",
    terms_invoice: "Betaling binnen 30 dagen na factuurdatum, tenzij schriftelijk anders overeengekomen.",
    terms_quote: "Offerte geldig gedurende 14 dagen, tenzij anders vermeld.",
    page: "Pagina"
  }
};

const getPdfText = (lang, key) => PDF_TRANSLATIONS[lang]?.[key] || PDF_TRANSLATIONS.fr[key];
...
export const generateDocumentPDF = (document, options = false) => {
  const openInNewTab = typeof options === 'boolean' ? options : !!options.openInNewTab;
  const returnBlob = typeof options === 'object' ? !!options.returnBlob : false;

  // Determine language (default to FR if not set)
  const lang = document.customer_language || 'fr';
  const t = (key) => getPdfText(lang, key);
  ...
  doc.setFontSize(7);
  doc.setFont("helvetica", "bold");
  doc.text(lang === 'nl' ? "Openingsuren" : "Heures d'ouverture", pageWidth - margin, y + 5, { align: "right" });

  doc.setFont("helvetica", "normal");
  doc.text(lang === 'nl' ? "Ma-Vr: 08u00 - 17u30" : "Lu-Ve: 08h00 - 17h30", pageWidth - margin, y + 10, { align: "right" });
  doc.text(lang === 'nl' ? "Za: 09u00 - 13u00" : "Sa: 09h00 - 13h00", pageWidth - margin, y + 14, { align: "right" });
  ...
  doc.setFontSize(7);
  doc.setFont("helvetica", "bold");
  doc.text(t('date'), margin + 2, y + 4);
  doc.text(`N° ${document.doc_type === 'quote' ? t('quote') : t('invoice')}`, margin + colWidth + 2, y + 4);
  doc.text(t('ref'), margin + colWidth * 2 + 2, y + 4);
  doc.text(t('due_date'), margin + colWidth * 3 + 2, y + 4);
  ...
  doc.text("REF", margin + 3, y + 5);
  doc.text(t('description').toUpperCase(), margin + 3, y + 9);

  // Column: QUANTITÉ / HOEVEELHEID
  doc.text(t('qty').toUpperCase(), pageWidth - 85, y + 5);

  // Column: PRIX UNIT. / EENHEIDS
  doc.text(t('price').toUpperCase(), pageWidth - 65, y + 5);

  // Column: REMISE / KORTING
  doc.text(t('discount').toUpperCase(), pageWidth - 45, y + 5);

  // Column: TVA / BTW
  doc.text(t('vat').toUpperCase(), pageWidth - 27, y + 5);

  // Column: TOTAL TTC / TOTAAL INCL
  doc.text(t('total_ttc'), pageWidth - margin - 3, y + 5, { align: "right" });
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text(t('total_ttc'), totalsX + 5, y + 28 + yOffset);
  doc.text(`€${total.toFixed(2)}`, totalsX + totalsWidth - 5, y + 28 + yOffset, { align: "right" });
  ...
  if (document.status === "unpaid" || document.status === "partially_paid") {
    ...
    doc.saveGraphicsState();
    doc.setGState(new doc.GState({ opacity: 0.3 }));
    doc.text(t('unpaid'), pageWidth / 2, 150, { align: "center", angle: 45 });
    doc.restoreGraphicsState();
    ...
    doc.text(t('paid'), pageWidth / 2, 150, { align: "center", angle: 45 });
    doc.restoreGraphicsState();
    ...
    doc.saveGraphicsState();
    doc.setGState(new doc.GState({ opacity: 0.2 }));
    doc.text(t('draft'), pageWidth / 2, 150, { align: "center", angle: 45 });
    doc.restoreGraphicsState();
  }
  ...
  if (document.doc_type === 'invoice') {
    ...
    // Second line for bilingual safety if needed, or just rely on t()
    // For now, we use the specific language term.
    doc.setFontSize(6.5);
    doc.text(
      "Conditions générales : voir page 2 (CGV) / Algemene voorwaarden: zie pagina 2 (AV) - https://alphanco.be/terms",
      pageWidth / 2,
      footerY + 13,
      { align: "center", maxWidth: pageWidth - 2 * margin }
    );
