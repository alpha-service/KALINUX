...
const DOC_TYPE_CONFIG = {
  ...
  return: { label: "Retour / Retour", icon: ArrowRight, color: "bg-orange-100 text-orange-800" },
};
...
export default function DocumentsHub() {
  ...
  const [allDocuments, setAllDocuments] = useState([]); // For stats calculation
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [activeTab, setActiveTab] = useState("all");

  // Fetch all documents for stats (runs once on mount)
  useEffect(() => {
    const fetchAllDocs = async () => {
      try {
        const response = await axios.get(`${API}/documents?limit=500`);
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      ...
      // Also update allDocuments when fetching
      if (activeTab === "all" && statusFilter === "all") {
        setAllDocuments(response.data);
      }
    } catch (error) {
      toast.error("Erreur de chargement", { duration: 2000 });
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleConvertToInvoice = async (doc) => {
    try {
      ...
      toast.success(`Converti en facture: ${response.data.number}`, { duration: 2000 });
      fetchDocuments();
      // Refresh all documents for stats
      const allDocsResponse = await axios.get(`${API}/documents?limit=500`);
      setAllDocuments(allDocsResponse.data);
      navigate(`/documents/${response.data.id}`);
    } catch (error) {
      toast.error("Erreur lors de la conversion", { duration: 2000 });
    }
  };
  ...
  const handleSendPeppol = async (doc) => {
    try {
      ...
      toast.error(errorMsg, { id: "peppol-send", duration: 2000 });
    }
  };
  ...
  const handleDownloadPDF = async (doc) => {
    try {
      ...
      toast.success("PDF téléchargé", { duration: 2000 });
    } catch (error) {
      console.error("PDF download error:", error);
      toast.error("Erreur lors du téléchargement du PDF", { duration: 2000 });
    }
  };
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-4 text-sm text-muted-foreground">
                            {new Date(doc.date || doc.created_at).toLocaleDateString("fr-BE")}
                          </td>
                          <td className="p-4">
                            {doc.customer_name || "—"}
                          </td>
