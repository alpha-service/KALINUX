...
export default function POSScreen() {
  ...
  const [globalDiscount, setGlobalDiscount] = useState({ type: null, value: 0, target: "htva" });
  const [showDiscountDialog, setShowDiscountDialog] = useState(false);
  const [discountInput, setDiscountInput] = useState({ type: "percent", value: "", target: "htva" });
    ...
    if (selectedCategory) {
      ...
      const getAllDescendantIds = (catId) => {
        ...
        directChildren.forEach(childId => {
          allDescendants.push(...getAllDescendantIds(childId));
        });
        return allDescendants;
      };

      // Include products from selected category AND all its descendants
      const descendantIds = getAllDescendantIds(selectedCategory.id);
      const categoryProducts = allProducts.filter(p =>
        p.category_id === selectedCategory.id || descendantIds.includes(p.category_id)
      );
      setProducts(categoryProducts);

      // Check if category has subcategories
      const hasSubCats = categories.some(c => c.parent_id === selectedCategory.id);
      if (!hasSubCats) {
        // No subcategories, show products view
        setPosViewMode("products");
      } else {
        // Has subcategories, show collections view (will also show products below)
  ...
  const filteredCategories = useMemo(() => {
    ...
    // Show only sub-categories of current selected category (or root if null)
    const currentParentId = selectedCategory ? selectedCategory.id : null;
    return categories.filter(cat => (cat.parent_id || null) === currentParentId);
  ...
  const goBackToParent = useCallback(() => {
    if (!selectedCategory) return;
    const parent = categories.find(c => c.id === selectedCategory.parent_id);
          ...
          toast.success(
            <div className="flex items-center gap-2">
              {foundProduct.image_url && (
                <img src={getImageUrl(foundProduct.image_url)} alt="" className="w-8 h-8 rounded object-cover" />
              )}
              <span>✓ {itemName}</span>
            </div>,
            { duration: 2000 }
  ...
  const totals = useMemo(() => {
    ...
    if (globalDiscount.value > 0) {
      if (globalDiscount.target === "ttc") {
        // Discount on TTC
        const totalBeforeGlobal = subtotal + vatTotal;
        let discountAmountTTC = 0;
        if (globalDiscount.type === "percent") {
          discountAmountTTC = totalBeforeGlobal * (globalDiscount.value / 100);
        } else {
          discountAmountTTC = Math.min(globalDiscount.value, totalBeforeGlobal);
        }
        const totalAfterGlobal = totalBeforeGlobal - discountAmountTTC;
        // Recalculate HTVA and VAT from new TTC (assuming 21%)
        subtotal = totalAfterGlobal / 1.21;
        vatTotal = totalAfterGlobal - subtotal;
      } else {
        // Discount on HTVA (Default)
        let discountAmountHT = 0;
        if (globalDiscount.type === "percent") {
          discountAmountHT = subtotal * (globalDiscount.value / 100);
        } else {
          discountAmountHT = Math.min(globalDiscount.value, subtotal);
        }
        subtotal -= discountAmountHT;
  ...
  const handlePaymentComplete = async (payments) => {
    try {
      const saleData = {
        ...
        customer_language: selectedCustomer?.language || "fr",
        items: cart.map(item => ({
  ...
  const handleApplyDiscount = () => {
    if (discountInput.value) {
      ...
      // Validation
      if (discountInput.type === "percent" && val > 100) {
        toast.error("Le pourcentage ne peut pas dépasser 100%");
        return;
      }

      if (discountInput.type === "fixed") {
        const currentTotals = totals; // totals is from useMemo
        const limit = discountInput.target === "ttc" ? parseFloat(currentTotals.total) : parseFloat(currentTotals.subtotal);
        if (val > limit) {
          toast.error(`Le montant ne peut pas dépasser le total (${discountInput.target.toUpperCase()})`);
          return;
        }
      }
          ...
          {globalDiscount.value > 0 && (
            <div className="flex items-center ml-1.5">
              ...
              <Button
                variant="ghost"
                size="sm"
                className="h-4 w-4 p-0 ml-1 text-red-500 hover:text-red-700 hover:bg-transparent"
                onClick={(e) => {
                  e.stopPropagation();
                  setGlobalDiscount({ type: null, value: 0, target: "htva" });
                  toast.success("Remise supprimée");
                }}
              >
          ...
          {posViewMode === "collections" && (
            <>
              ...
              {(selectedCategory || categoryPath.length > 0) && (
                <div className="px-3 py-2 flex items-center gap-1.5 overflow-x-auto scrollbar-none bg-white/50 border-b border-slate-100">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-7 px-2 text-xs text-muted-foreground hover:text-brand-navy"
                  ...
                  {categoryPath.map((cat, idx) => (
                    <div key={cat.id} className="flex items-center gap-1.5 shrink-0">
                      ...
                      <Button
                        ...
                        className={cn(
                          "h-7 px-2 text-xs",
                          idx === categoryPath.length - 1 ? "font-bold text-brand-navy pointer-events-none" : "text-muted-foreground hover:text-brand-navy"
                        )}
              ...
              <ScrollArea className={cn(
                ...
                {/* Category Breadcrumbs - Inside scroll area (top) or separate? Kept inside for simplicity or move above? 
                    Actually, let's keep breadcrumbs sticky at top if possible or just at top of flow.
                    The previous code had breadcrumbs OUTSIDE ScrollArea in line 1797. 
                    Wait, looking at lines 1797-1824, it was OUTSIDE. 
                    I should verify where I am editing. 
                    The replacement target was `ScrollArea` at 1826 and `div` at 1853.
                */}
                ...
                <div className="p-3 space-y-4">
                  ...
                  {selectedCategory && filteredProducts.length > 0 && (
                    <div className="pt-4 border-t border-slate-100">
                      <h3 className="text-sm font-semibold text-brand-navy mb-3 px-1">
                        {t('pos_products')} ({filteredProducts.length})
                      </h3>
                      <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-2">
                        {filteredProducts.map((product) => (
                          <ProductCard
                            ...
                            gridSize="small" // Use small grid for mixed view to save space? Or stick to default? Let's use logic
                          />
                        ))}
                      </div>
                    </div>
                  ...
                  {!loading && filteredCategories.length === 0 && (!selectedCategory || filteredProducts.length === 0) && (
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="flex flex-col gap-3">
              <Label className="text-xs font-medium uppercase text-muted-foreground">{t('pos_type')}</Label>
              <div className="flex gap-2">
                <Button
                  variant={discountInput.type === "percent" ? "default" : "outline"}
                  style={discountInput.type === "percent" ? { backgroundColor: colors.accent } : {}}
                  className={cn("flex-1", discountInput.type === "percent" ? "text-white" : "")}
                ...
                <Button
                  ...
                  className={cn("flex-1", discountInput.type === "fixed" ? "text-white" : "")}
            ...
            <div className="flex flex-col gap-3">
              <Label className="text-xs font-medium uppercase text-muted-foreground">{t('pos_discount_target')}</Label>
              <div className="flex gap-2">
                <Button
                  variant={discountInput.target === "htva" ? "default" : "outline"}
                  className={cn("flex-1", discountInput.target === "htva" ? "bg-slate-800 text-white" : "")}
                  onClick={() => setDiscountInput(prev => ({ ...prev, target: "htva" }))}
                >
                  {t('pos_target_htva')}
                </Button>
                <Button
                  variant={discountInput.target === "ttc" ? "default" : "outline"}
                  className={cn("flex-1", discountInput.target === "ttc" ? "bg-slate-800 text-white" : "")}
                  onClick={() => setDiscountInput(prev => ({ ...prev, target: "ttc" }))}
                >
                  {t('pos_target_ttc')}
                </Button>
              </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="discount-value">{t('pos_discount_amount')}</Label>
              <Input
                id="discount-value"
                type="number"
                placeholder="0.00"
                ...
                onChange={(e) => {
                  ...
                  // Validate max 100% for percentage
                  if (discountInput.type === "percent" && val > 100) {
                    toast.error("La remise ne peut pas dépasser 100%");
                    return;
                  }
                  setDiscountInput(prev => ({ ...prev, value: e.target.value }));
                }}
              ...
              <p className="text-[10px] text-muted-foreground italic">
                {discountInput.type === "percent"
                  ? "Limite: 100% maximum"
                  : `Limite: €${discountInput.target === "ttc" ? totals.total : totals.subtotal} maximum`
                }
              </p>
            </div>
          </div>
          <DialogFooter className="flex justify-between sm:justify-between items-center gap-2">
            <div className="flex gap-2">
              <Button
                ...
                onClick={() => {
                  setGlobalDiscount({ type: null, value: 0, target: "htva" });
                  setDiscountInput({ type: "percent", value: "", target: "htva" });
                  setShowDiscountDialog(false);
                  toast.success("Remise supprimée");
                }}
                disabled={globalDiscount.value === 0}
              >
                {t('delete')}
              </Button>
            </div>
